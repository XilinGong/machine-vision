...
class image_stitching:
    def __init__(self, file_name):
        ...
        for format in self.config.image_format_list:
            if os.path.exists(f'./src/image/image_stitching/{self.file_name}/{self.file_name}_1.{format}'):
                self.work_path = f'./src/image/image_stitching/{self.file_name}'
...
# public:
    ...
    def stitch(self):
        if self.config.stitch_style=='successive':
            ...
            if len(images) >= 2:
                ...
                if self.config.cylindrical_projection_run:
                    if self.config.cut_style == 'reel':
                        idx = np.argwhere(np.any(
                            image_fixed[:, int(image_fixed.shape[1] / 3):int(image_fixed.shape[1] / 3 * 2), :] == 0,
                            axis=1))
                        image_fixed = np.delete(image_fixed, idx, axis=0)
                self.show_picture(image_fixed, 'stitching result')
            ...
            cnt = 2
            image_fixed = images[4]
            image_variable = images[3]
            key_points_variable, descriptors_variable = self.surf(image_variable)
            key_points_fixed, descriptors_fixed = self.surf(image_fixed)
            ...
            image_fixed = images[2]
            for image_variable in current[:]:
                cnt += 1
                # 计算出两张图片的关键点坐标列表与关键点描述子列表
                key_points_variable, descriptors_variable = self.surf(image_variable)
                key_points_fixed, descriptors_fixed = self.surf(image_fixed)
            ...
            if self.config.cylindrical_projection_run:
                if self.config.cut_style == 'reel':
                    idx = np.argwhere(np.any(
                        image_fixed[:, int(image_fixed.shape[1] / 3):int(image_fixed.shape[1] / 3 * 2),:] == 0, axis=1))
                    image_fixed = np.delete(image_fixed, idx, axis=0)
            self.show_picture(image_fixed, 'stitching result')
            cv2.imwrite(f'{self.stitch_result_path}/result.{self.config.imwrite_format}', image_fixed)
            return image_fixed
    ...
    def stitch_images(self, image_variable, image_fixed, Homography_matrix, i):
        ...
        # 掩膜衰减
        if config.mask_decay:
            for c in range(3):
                bi_overlap = bi_overlap_canvas[:, :, c]
                bi_decay = np.zeros(bi_overlap.shape)
                for i in range(bi_decay.shape[0]):
                    if np.array(np.where(bi_overlap[i] == 1)).size:
                        left = np.where(bi_overlap[i] == 1)[0][0]
                        right = np.where(bi_overlap[i] == 1)[0][-1]
                        delt = right - left
                        left_ = int(left + delt * config.w / 2)
                        right_ = int(right - delt * config.w / 2)
                        bi_decay[i, left_+1:right_] = 1

                        min_fixed = np.where(bi_fixed_canvas[i] == 1)[0][0]
                        if(min_fixed < left):
                            bi_fixed_canvas[i, right_:right+1, c] = 0
                            bi_variable_canvas[i, left:left_+1, c] = 0
                        else:
                            bi_variable_canvas[i, right_:right+1, c] = 0
                            bi_fixed_canvas[i, left:left_+1, c] = 0
                bi_overlap_canvas[:, :, c] = bi_decay
        ...
        if self.config.p_trans_style=='average':
            ...
            if self.config.p_trans_show:
                ...
                cv2.imwrite("Fixed picture display ratio.jpg", p_fixed*bi_overlap_canvas*255)
                cv2.imwrite("Variable picture display ratio.jpg", p_variable*bi_overlap_canvas*255)
        ...
        if self.config.image_fusion_run:
            if self.config.multi_band_blend:
                ...
                stitching_result += (bi_fixed_canvas - bi_overlap_canvas) * fixed_canvas + (bi_variable_canvas - bi_overlap_canvas) * variable_canvas
